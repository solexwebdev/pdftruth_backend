...
export class Migration20250108140952 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "roles" ("id" varchar(255) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" text check ("name" in ('super_admin', 'owner', 'manager', 'viewer')) not null, constraint "roles_pkey" primary key ("id"));`,
    );
    ...
    this.addSql(
      `create table "socials" ("id" varchar(255) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "sub" varchar(255) not null, "vendor" text check ("vendor" in ('google', 'apple')) not null, "user_id" varchar(255) not null, constraint "socials_pkey" primary key ("id"));`,
    );
